...
function draw() {
  ...
  const grad = ctx.createRadialGradient(bx - 2, by - 2, 0, bx, by, BALL_R);
...
function ballHitPaddle() {
  ...
  const angle = (hitPos - 0.5) * Math.PI * 0.7; // max spread angle
...
function ballHitBrick(brick) {
  ...
  const prevX = bx - ball.dx, prevY = by - ball.dy;

  // Determine which side was hit
  if (prevX + r <= brick.x) ball.dx = -Math.abs(ball.dx);
  else if (prevX - r >= brick.x + brick.w) ball.dx = Math.abs(ball.dx);
  else if (prevY + r <= brick.y) ball.dy = -Math.abs(ball.dy);
  else if (prevY - r >= brick.y + brick.h) ball.dy = Math.abs(ball.dy);
  else ball.dy *= -1;
}
